...
###################################################
# (2) torch model of 5-fold cross-validation splits
# IMPORTANT: We will reduce the training dataset from 1000 image in each of 40 class
# to only 0.6*1000=600 images in each class

split = []

total_length = len(fileList)
reduced_length = int(total_length*0.6)
fold_num = args.fold
...
s = int(reduced_length/number_of_classes) # samples_per_class
s_fold = int(s/fold_num) # samples_per_class_per_fold
half_s_fold = int(s_fold/2)
print("Total Length of file list: ", total_length)
print("Reduced length of file list: ", reduced_length)
    ...
    for k, v in classes_dict.items():
        ...
        for j in sample_idx:
            if j in subsample1_idx:
                spliti["val"].append(v[j]) # length=2400
            elif j in subsample2_idx:
                spliti["test"].append(v[j]) #length=2400
            else:
                spliti["train"].append(v[j]) #length=19200
...
mydict = {"splits": split}
dataset_name = args.output_path.split(".")[0]+"_splits_60_percent.pth"
