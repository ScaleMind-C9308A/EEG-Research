...
class classifier_EEGNet(nn.Module):
    def __init__(self, spatial, temporal):
        ...
        self.network = nn.Sequential( #input(batch, 1, 96, 512)
            ### FIRST BLOCK
            nn.ZeroPad2d((first_padding, first_padding-1, 0, 0)), # (1, 1,)
            ...
            nn.Dropout())
        self.fc = nn.Linear(F2*(temporal//32), 40) #input size=16x(512/8)=256; output size=40

    def forward(self, x):
        #input size of x: (batch, 96, 512)
        print("Input size: ",x.size())
        x = x.unsqueeze(0).permute(1, 0, 2, 3) #(batch, 1, 96, 512)
        print("After permute: ",x.size())
        x = self.network(x) #(batch, 16, 1, 16)
        print("After network: ",x.size())
        x = x.view(x.size()[0], -1) #(batch, 16x16)
