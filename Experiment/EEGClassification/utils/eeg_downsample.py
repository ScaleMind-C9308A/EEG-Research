...
output_filename = 'eeg_55_95_std_downsampled_128Hz_resample.pth'
...
def eeg_downsample(eeg_data, time_low=20, time_high=460, method='resample_poly'):
    """
    Downsample EEG data from 440Hz to 128Hz.
    Args
        eeg_data(np.array): EEG data (128, 440)
        time_low(float): Lowest time value of eeg segment
        time_high(float): highest time value of eeg segment
        method(str): Downsample method (resample_poly | resample)
    """
    ...
    if method == 'resample':
        eeg_data = signal.resample(eeg_data, num=128, axis=1) # (128, 128)
...
for sample in eeg_dataset:
    sample['eeg'] = eeg_downsample(sample['eeg'], method='resample')
